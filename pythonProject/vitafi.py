...
def main_menu(stdscr):
    ...
    def impSave():
        file = open("vitafi_save.txt","r")
        tick_arr = file.readlines()
        for x in range(len(tick_arr)):
            price_arr.append(getPrice(tick_arr[x]))
    ...
    while (k != ord('q')):
        ...
        imp = "Import Last Save"[:width-1]
        start_x_ticker, start_x_analyze, start_x_imp = 0,0,0
        ...
        if y_pos == 2 and x_pos == 0:
            stdscr.addstr(start_y + 2, start_x_imp, imp, curses.A_STANDOUT)
            inprompt = "Import last save"
            if k == 10:
                impSave()
        else:
            stdscr.addstr(start_y + 2, start_x_imp, imp)
                ...
                if y_pos == x and x_pos >= 1 and x_pos <= 3:
                    ...
                    if menu == True:
                          ...
                          if x_pos == 1:
                            ...
                            if k == 10:
                                inprompt = "Analyze!".format(tick_arr[x])
                                while curses.wrapper(vitanifi.analyze_menu) != 0:
                                    curses.wrapper(vitanifi.analyze_menu)
                                stdscr.refresh()
                                menu = False
                else:
